...
mkdir -p logs
QUEUE_LOG="logs/l2t-chemistry-sdpo-tr-et-after-${WAIT_PID}-$(date +%Y%m%d-%H%M%S).log"
echo "$$" > logs/l2t-chemistry-sdpo-tr-et-after.pid
echo "$QUEUE_LOG" > logs/l2t-chemistry-sdpo-tr-et-after.logpath

{
    if [[ "$WAIT_PID" == "0" || "$WAIT_PID" == "now" ]]; then
        echo "[$(date -u +"%F %T UTC")] Starting immediately before chemistry SDPO-TR/SDPO-TR+ET."
    else
        echo "[$(date -u +"%F %T UTC")] Waiting for PID ${WAIT_PID} before chemistry SDPO-TR/SDPO-TR+ET."
    ...
    echo "[$(date -u +"%F %T UTC")] Wait condition satisfied. Starting chemistry SDPO+TR baseline."
    ...
    echo "[$(date -u +"%F %T UTC")] Starting chemistry SDPO+TR+ET."
    ...
    echo "[$(date -u +"%F %T UTC")] Chemistry SDPO+TR/SDPO+TR+ET queue finished."
} >> "$QUEUE_LOG" 2>&1
